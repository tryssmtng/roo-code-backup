...
type AdvancedSettingsProps = HTMLAttributes<HTMLDivElement> & {
	...
	diffEnabled?: boolean
	promptAutoEnhanceEnabled?: boolean
	fuzzyMatchThreshold?: number
	setCachedStateField: SetCachedStateField<"rateLimitSeconds" | "diffEnabled" | "fuzzyMatchThreshold" | "promptAutoEnhanceEnabled">
	...
	return (
		<div className={cn("flex flex-col gap-2", className)} {...props}>
			...
			<Section>
				<div>
					<VSCodeCheckbox
						checked={promptAutoEnhanceEnabled}
						onChange={(e: any) => setCachedStateField("promptAutoEnhanceEnabled", e.target.checked)}>
						<span className="font-medium">{t("settings:advanced.promptAutoEnhance.label")}</span>
					</VSCodeCheckbox>
					<p className="text-vscode-descriptionForeground text-sm mt-0">
						{t("settings:advanced.promptAutoEnhance.description")}
					</p>
				</div>
