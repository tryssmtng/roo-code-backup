			...
			async (message: WebviewMessage) => {
				switch (message.type) {
					...
					case "enhancementApiConfigId":
						...
						await this.postStateToWebview()
						break
					case "promptAutoEnhanceEnabled":
						await this.updateGlobalState("promptAutoEnhanceEnabled", message.bool ?? false)
						await this.postStateToWebview()
						break
					case "autoSpeakEnabled":
						await this.updateGlobalState("autoSpeakEnabled", message.bool ?? false)
						await this.postStateToWebview()
						break
					case "autoSpeakVoiceModel":
						await this.updateGlobalState("autoSpeakVoiceModel", message.text ?? "alloy")
						await this.postStateToWebview()
						break
					case "textToSpeech":
						if (message.text && message.voiceModel) {
							try {
								// Get the OpenAI API key from secrets
								const apiKey = await this.context.secrets.get("openAiApiKey")
								if (!apiKey) {
									vscode.window.showErrorMessage("OpenAI API key not found. Please add it in the settings.")
									break
								}
								...
								// Send the audio data back to the webview
								await this.postMessageToWebview({
									type: "playAudio",
									text: audioData
								})
								...
								this.outputChannel.appendLine(
									`Error in text-to-speech: ${error instanceof Error ? error.message : String(error)}`
								)
								vscode.window.showErrorMessage(`Text-to-speech error: ${error instanceof Error ? error.message : String(error)}`)
							}
						}
						break
					case "storeSecret":
						if (message.key && message.value) {
							try {
								await this.context.secrets.store(message.key, message.value)
							} catch (error) {
								this.outputChannel.appendLine(
									`Error storing secret: ${error instanceof Error ? error.message : String(error)}`
								)
								vscode.window.showErrorMessage(`Error storing secret: ${error instanceof Error ? error.message : String(error)}`)
							}
						}
						break
